...
int main(int argc, char* argv[])
{
        ...
        if(argc < 2){		//No arguments; start test suite
			...
			compiler.Add_Sourcefile((char*)"C:\\testsource.au3");
			compiler.DumpTokens_ToFile("toks.txt");
			compiler.Compile();
			compiler.PrintAST();
			std::string temp;
			std::cin >> temp;
			...
			for(;cmdpos<argc;cmdpos++){
				...
				case '-':
					...
					else if((*(argv[cmdpos]+1) == 'p')||(*(argv[cmdpos]+1) == 'P')) //Dump code to stdout.
						compiler.GenerateCode_ToSTDOUT();
					...
					else if((*(argv[cmdpos]+1) == 's')||(*(argv[cmdpos]+1) == 'S')) //Dump Symbol Table to a file
					{
						char filename[255];
						strcpy( filename, argv[cmdpos+1]);
						compiler.DumpSymTable_ToFile(filename);
						cmdpos++;
					}
